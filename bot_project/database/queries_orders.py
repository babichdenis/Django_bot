...
async def get_order_by_id(order_id: int):
    ...
    try:
        ...
        if order:
            return dict(order)  # Преобразование RowProxy в словарь
        else:
            return None
    except Exception as e:
...
async def update_order_status(order_id: int, total: float):
    ...
    try:
        logger.debug(f"Запрос в БД: обновление общей суммы заказа - order_id={order_id}, total={total}")
        ...
        logger.debug(f"Обновлена общая сумма заказа для order_id={order_id} до {total}")
        return True
    except Exception as e:
...
async def update_order_payment_status(order_id: int, status: str):
    ...
    try:
        logger.debug(f"Запрос в БД: обновление статуса оплаты заказа - order_id={order_id}, status={status}")
        ...
        logger.debug(f"Обновлен статус оплаты заказа для order_id={order_id} до {status}")
        return True
    except Exception as e:
...
async def get_cart_item(telegram_id: int, product_id: int):
    ...
    try:
        logger.debug(f"Запрос в БД: получение товара из корзины для telegram_id={telegram_id}, product_id={product_id}")
        ...
        if result:
            ...
            result['price'] = float(result['price']) if isinstance(result['price'], Decimal) else result['price']
            result['total'] = float(result['total']) if isinstance(result['total'], Decimal) else result['total']
            return result
        else:
            return None
    except Exception as e:
